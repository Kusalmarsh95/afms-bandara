...
<main>
    <div id="details" class="clearfix">
        <div id="topic">
            <h2 class="name"> <u>Full Withdrawals - Bank Deposit List</u></h2>
        </div>
    </div>
    <table class="border">
        <thead>
        <tr>
            ...
            <th class="center border" scope="col">Total Fund</th>
            <th class="center border" scope="col">Deductions</th>
        ...
        @foreach ($fullWithdrawals as $fullWithdrawal)
            <tr>
                ...
                <td class="border">{{ $fullWithdrawal->membership->regimental_number }}</td>
                <td class="border">{{ $fullWithdrawal->membership->ranks->rank_name ?? '--' }} {{ $fullWithdrawal->membership->name }}</td>
                <td class="border">{{ $fullWithdrawal->membership->regiments->regiment_name ?? '--' }}</td>
                <td class="right border">{{ number_format($fullWithdrawal->fullWithdrawal->fund_balance ?? '0', 2) }}</td>
                <td class="right border">{{ number_format(($fullWithdrawal->fullWithdrawal->fund_balance - $fullWithdrawal->fullWithdrawal->eligible_amount)?? '0', 2) }}</td>
                <td class="right border">{{ number_format($fullWithdrawal->fullWithdrawal->eligible_amount ?? '0', 2) }}</td>
            </tr>
        @endforeach
        <tr>
